...
uint8_t at24c02_ReadBytes(uint8_t *_pReadBuf, uint16_t _usAddress, uint16_t _usSize)
{
	...
	i2c_SendByte(DEV_ADDR | I2C_WR);	/* 此处是写指令 */
	...
	i2c_SendByte(DEV_ADDR | I2C_RD);	/* 此处是读指令 */
...
static void at24c02_Delay(__IO uint32_t nCount)	 //简单的延时函数
{
	for(; nCount != 0; nCount--);
}
...
/*------------------------------------------------------------------------------------*/  
  if (at24c02_WriteBytes(write_buf, 0, SIZE) == 0)
	{
		printf("写eeprom出错！\r\n");
		return 0;
	}
  ...
  /*写完之后需要适当的延时再去读，不然会出错*/
	at24c02_Delay(0x0FFFFF);
/*-----------------------------------------------------------------------------------*/
  if (at24c02_ReadBytes(read_buf, 0, SIZE) == 0)
	{
		printf("读eeprom出错！\r\n");
		return 0;
	}
